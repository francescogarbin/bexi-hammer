...
class FileRow(Gtk.ListBoxRow):
    ...
    def create_from_request_context(ctx):
        ...
        service_code = ctx.get_attribute('cod_servizio', "Servizio: N/A")
        flow_code = ctx.get_attribute('cod_flusso', "Flusso: N/A")
    ...
    def set_status_image(self, request_context_status):
        ...
        if RequestContextStatus.Idle == request_context_status: 
            ...
            self._status_image.set_from_stock(Gtk.STOCK_DIALOG_WARNING,
                                              icon_size)
            self._status_image.show()
            ...
            self._status_image.set_from_stock(Gtk.STOCK_DIALOG_ERROR,
                                              icon_size)
            self._status_image.show()
        elif RequestContextStatus.Undefined == request_context_status:
            self._status_image.set_from_stock(Gtk.STOCK_DIALOG_QUESTION,
                                              icon_size)
            self._status_image.show()
        else:
            self._status_image.hide()
